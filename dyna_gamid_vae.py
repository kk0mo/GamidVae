    ...
    def calculate_actor_distance(self, next_state):
        ...
        if self.total_it % 5000 == 0:
            ...
            if min_q < 0.8 * max_q:
                ...
                actor1, actor2 = self.get_idx_pair(min_dist_idx, n_actors)
                q1 = q_values[:, actor1].mean()
                q2 = q_values[:, actor2].mean()
                actor_to_elimi = actor1 if q1 < q2 else actor2
            if actor_to_elimi is not None:
